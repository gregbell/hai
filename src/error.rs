...
pub fn handle_error(err: anyhow::Error) -> ! {
    ...
    if err_string.contains("No models configured") || err_string.contains("Model") && err_string.contains("not found in config") {
        eprintln!("\nPlease check your configuration file at ~/.config/hai/config.toml");
        eprintln!("Make sure you have configured at least one model and that it has a valid API key.");
        eprintln!("You can copy the example configuration from config.toml.example.");
    } else if err_string.contains("Failed to send request to API") || err_string.contains("Failed to parse API response") {
